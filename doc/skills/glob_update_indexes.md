...
> Глобальный навык (префикс `glob_`), раздается во все репозитории. Работает только по локальному репозиторию, кросс-репо ничего не знает. В хабе `project_standards` не применяется - там сводный индекс `_index_skills_hub.md` ведет `hub_sync_indexes`.
...
- Добавлен, удален или переименован навык в корне `doc/skills/` репозитория
...
3. В хабе `project_standards` скрипт ничего не делает и сообщает, что сводный индекс ведет `hub_sync_indexes`.
4. Вывод передать пользователю: какие навыки попали в индекс, путь к обновленному файлу.
...
def main() -> None:
    ...
    print(f"Обновлен {INDEX_FILE} - навыков: {len(skills)}")
    for p in skills:
        print(f"  {p.name}")
...
- Пользователю выдан отчет со списком навыков и путем к файлу

## Примеры

1. Пользователь: «обнови индекс навыков» -> агент запускает скрипт, пересобирает `_index_skills_repo.md`, выдает список навыков.
...
- Не запускать в хабе `project_standards` (там сводный индекс ведет `hub_sync_indexes`)
