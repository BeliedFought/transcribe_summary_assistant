# Стандарт деплоя Python-проекта как системного инструмента. Версия 4.12.1
...
[project.scripts]
command = "tool_name.module:function"
...
- **`[project.scripts]`** - создает CLI-команду. Формат: `имя_команды = "путь.до.модуля:функция"`; путь начинается с имени пакета, как он импортируется после установки. После установки wrapper-скрипт попадает в каталог пользователя. Пример: `ghupd = "tool_name.main:main"` создаст команду `ghupd`, которая вызывает `tool_name.main.main()`.
- **`py-modules`** - для одиночных модулей вне пакета. В модели src-layout точка входа лежит внутри пакета (`src/tool_name/main.py`), поэтому секция не используется.
- **`[tool.setuptools.packages.find]`** - задает src-layout: пакет `tool_name` лежит в каталоге `src/`. `where = ["src"]` - каталог поиска пакетов, `include = ["tool_name*"]` - собирать только пакет `tool_name`. Каталог `src` сам пакетом не является и в wheel не попадает: после установки код импортируется как `from tool_name.xxx import ...`, в site-packages - `tool_name/` (разделы 01.03.02-01.03.04).
