# Стандарт деплоя Python-проекта как системного инструмента. Версия 4.9.1
...
**1. Условия и сборка:** 1.01 Обязательные условия к проекту - 1.02 Структура pyproject.toml - 1.03 Упаковка non-Python файлов (1.03.01 MANIFEST.in - 1.03.02 package-data - 1.03.03 Доступ к data-файлам - 1.03.04 Правила)
...
```
recursive-include src/tool_name/sql *.sql
...
### 1.03.02. package-data в pyproject.toml

Рантайм-активы, читаемые установленным инструментом (SQL-файлы по `project_standards.md` 03.05), размещаются внутри пакета: `src/<tool_name>/sql/`. Для проектов с деплоем это размещение перекрывает корневую папку `sql/` из `project_standards.md` 01.01: в режиме разработки пакет лежит в `src/` репозитория, в установленном режиме - в site-packages изолированного venv, код читает файлы одним и тем же способом.
...
- Проверять доступность data-файлов после `uv tool install .`: содержимое wheel смотреть командой `unzip -l dist/<tool_name>-<version>-*.whl` - файлы должны лежать внутри пути пакета (`tool_name/sql/`)
...
Установочный скрипт `update.py` при обновлении выполняет единый алгоритм миграции. **Канон алгоритма - этот раздел**; overlay-документы (`deploy_lin_standards.md` 3.05, `deploy_win_standards.md` 5.05) содержат только платформенные пути, источник шаблонов и команды.

**Этап 0 - синхронизация версии в репозитории (только прямой деплой из репозитория).** Устраняет расхождение между `pyproject.toml` (поле `version`) и `config/config.ini` (параметр `version` секции `[app]`): сравнение по SemVer, большее значение записывается в источник с меньшим; нечисловые версии (`dev` и т.п.) - пропустить с сообщением. При отсутствии `config/config.ini` - сначала создать из `config/config.ini.example`. При деплое через архив этап отсутствует: версия зафиксирована в wheel на машине сборки.
...
Если prod-конфига еще нет - скопировать example и проставить `name`/`version` из источника.
